...
public class HiringManagerService{
	...
	public HiringManagerSearch search(String body){
		...
		if(hiringManagerSearchInput.getEntityTypesInput() == null){
			List<InputDTO> entityTypes = new ArrayList<InputDTO>();
			for(EntityType entityType : EntityType.values()){
				InputDTO inputDTO = new InputDTO(entityType.getLabel(), entityType.toString(), true);
				entityTypes.add(inputDTO);
			}
			hiringManagerSearchInput.setEntityTypesInput(entityTypes);
		}

		List<HiringManager> hiringManagerList = new ArrayList<HiringManager>();

		// Fetch placement partner hiring managers if 'Placement Partner' checkbox is checked
		if (hiringManagerSearchInput.getEntityTypesInput().stream().anyMatch(ti -> (ti.getSelected() && ti.getValue().equals(EntityType.PLACEMENT_PARTNER.toString())))) {
		...
		// Fetch school hiring managers if 'School' checkbox is checked
		if (hiringManagerSearchInput.getEntityTypesInput().stream().anyMatch(ti -> (ti.getSelected() && ti.getValue().equals(EntityType.SCHOOL.toString())))) {
